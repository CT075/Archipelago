...
class FE8Randomizer:
    ...
    def force_tag(self, amount: int, given_logic: str) -> None:
        ...
        self.randomize_chapter_unit(choosen.base, choosen.logic, False)

    def force_legal(self, logic: dict[str, Any], given_logic):
        # checks to see if a unit is legal to reroll
        # can't have any force job on them but "must_fight" as that only blocks 3 jobs
        # will have to add a case for if force healer is done
        # As Ephraim / L'arachel's have unpromoted units that can get the force fight tag

        for entry in logic[0]:
            if (
                entry.startswith("must_")
                and entry != given_logic
                and entry != "must_fight"
            ):
                return False
        return True
